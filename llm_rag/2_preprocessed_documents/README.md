...
```text
llm_rag/1_raw_documents/
```

This folder receives the outputs:

```text
llm_rag/2_preprocessed_documents/
```
...
It now resolves paths relative to the `llm_rag` folder, so it reads from `1_raw_documents/` and writes back into this directory consistently.
