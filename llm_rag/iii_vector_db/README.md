...
- `test_build_reference_db_unit.py`: validates the reference-build pipeline from helper functions up to `main()`. It checks record parsing and defaulting, metadata extraction, block-aware chunking, document flattening, JSONL exports, parent-table deduplication, strict embedding loading, Chroma build orchestration, summary counts, and the `--skip-chroma` main-path behavior.
- `test_threshold_lookup_unit.py`: validates the deterministic threshold route. It checks threshold-intent detection, criterion inference, requested-field parsing, compact answer assembly, field-specific answers for Criteria B, D, and E, field-only fallback queries such as `AOO` or `number of locations`, and safe `None` returns when a query does not map cleanly.
- `test_retrieval_engine_unit.py`: validates query-time retrieval helpers and routing. It checks tokenization and phrase matching, query-mode detection, candidate construction, table labeling, dense/sparse score merging, supporting-information coverage enforcement for Table 1 and Table 2, parent-context attachment, answer-scaffold generation, deterministic-threshold short-circuiting, and fallback into hybrid retrieval when exact lookup misses.

These tests use mocked dependencies so they run as fast unit tests without requiring a real Chroma index, embedding model, or live retrieval assets.

Run them from the repo root with:

```bash
python -m pytest -q llm_rag/iii_vector_db/test_build_reference_db_unit.py
python -m pytest -q llm_rag/iii_vector_db/test_threshold_lookup_unit.py
python -m pytest -q llm_rag/iii_vector_db/test_retrieval_engine_unit.py
```

Or run them together with:

```bash
python -m pytest -q llm_rag/iii_vector_db/test_build_reference_db_unit.py llm_rag/iii_vector_db/test_threshold_lookup_unit.py llm_rag/iii_vector_db/test_retrieval_engine_unit.py
```
