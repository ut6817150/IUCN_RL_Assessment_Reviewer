...
- `build_reference_db.py`: builds the retrieval assets from `2_preprocessed_documents/`
...
`build_reference_db.py` reads the `retrieval_blocks.jsonl` files created in `llm_rag/2_preprocessed_documents/` relative to the repo root.
...
```text
llm_rag/3_vector_db/chroma_db/reference_docs/
```
...
```bash
python llm_rag/3_vector_db/test_reference_db.py "What supporting information is required for a threatened species assessment?"
python llm_rag/3_vector_db/test_reference_db.py "What are the Criterion B thresholds for EOO and AOO?"
```
